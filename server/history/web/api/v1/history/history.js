...
router.get('/delta/:level/:source_id/:time_from/:limit', function (req, res) {
    res.setHeader('Content-Type', 'application/json');
    db.history.delta(req.params.level, req.params.source_id, req.params.time_from, req.params.limit).then(
        (rows) => {
            res.end( JSON.stringify(rows)  + '\n\r');
        },
        (error) => {
